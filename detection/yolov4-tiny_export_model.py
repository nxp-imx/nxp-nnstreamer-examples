...
parser.add_argument('--images_path', help='path to representative images for quantization', default=None)
...
def _conv_block(inp, convs, skip=False):
    ...
    for conv in convs:
        ...
        if conv['stride'] > 1:
            x = ZeroPadding2D(((1, 0), (1, 0)), name='zerop_' + str(conv['layer_idx']))(
...
def make_yolov4_tiny_model():
    input_image = Input(shape=(416, 416, 3), batch_size=1, name='input_0')
    # Layer 0
    ...
    x = _conv_block(layer_26, [{'filter': 256, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 27}])
    layer_27 = x
    ...
    x = _conv_block(x, [{'filter': 255, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 0, 'layer_idx': 29}])
    layer_29 = x

    # Layer 30, continue from layer_27
    x = _conv_block(layer_27, [{'filter': 128, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 30}])
    layer_30 = x
    # Layer 31
    x = UpSampling2D(size=(2, 2), name='upsamp_31', interpolation='bilinear')(x)
    layer_31 = x
    # Layer 32
    x = concatenate([layer_31, layer_23], axis=-1, name='concat_32')
    # Layer 33
    x = _conv_block(x, [{'filter': 256, 'kernel': 3, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 33}])
    # Layer 34, output of medium grid
    x = _conv_block(x, [{'filter': 255, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 0, 'layer_idx': 34}])
    layer_34 = x
    ...
    def load_weights(self, model):
        ...
        for i in range(35):
            try:
                ...
                print(f"loading weights of convolution #" + str(i) + "- nb parameters: " + str(nweights + filter))
...
def image_resize(image, resize_shape):
    ...
    real_w, real_h = (resize_w - temp_w) // 2, (resize_h - temp_h) // 2
    image_paded[real_h:temp_h + real_h, real_w:temp_w + real_w, :] = image_resized
